...
main(int argc, const char ** argv) {
    ...
    if (argc == 1) {
		...
		startx = starty = SZ/4;
		if (!empty(Pos(startx, starty))) startx++;
        } else {
            SZ = -SZ;
            for (unsigned i = 0; i < SZ; ++i) {
                pic.push_back(vector<char>(SZ, ' '));
	...
	if (q.empty()) std::swap(q, w); 
	++stepcnt;
	cur = q.front(); q.pop_front();
	if (!empty(cur)) continue;
	while (cur.first > 0 && empty(cur-Pos(1, 0)))
		cur.first -= 1;
	bool up = true, down = true;
	while (cur.first < pic[cur.second].size() && empty(cur)) {
		fill(cur);
		if (cur.second + 1 < pic.size()) {
			if (up && empty(cur + Pos(0, 1)) && q.size() < 100) {
				q.push_back(cur + Pos(0, 1));
			}
			up = !empty(cur + Pos(0, 1));
		}
		if (cur.second > 0) {
			if (down && empty(cur - Pos(0, 1)) && q.size() < 100) {
				w.push_back(cur - Pos(0, 1));
			}
			down = !empty(cur - Pos(0,1));
		}
		cur.first++;
	}
	show(); // getch();
	maxsz = std::max(maxsz, q.size()+w.size());
    }
    show();
    getch();
    endwin();
